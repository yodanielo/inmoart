...
function relativeDate($posted_date) {
    ...
    if ($month == "02") { // february
        ...
        if ($leapYear)
            $month_in_seconds = 2505600; // leap year
 else
        ...
        if ($month == "04" or
            ...
            $month_in_seconds = 2678400; // 31 day month;
...
function wp_pagenavi($before = '', $after = '', $prelabel = '', $nxtlabel = '', $pages_to_show = 5, $always_show = false) {
    ...
    if (empty($prelabel)) {
        $prelabel = '<strong>' . ARROW_FIRST . '</strong>';
    }
    if (empty($nxtlabel)) {
    ...
    if ($max_page > 1 || $always_show) {
        ...
        if ($paged >= ($pages_to_show - 1)) {
            echo '<a class="flecizq" href="' . str_replace('&', '&amp;', str_replace('&', '&amp;', get_pagenum_link())) . '">' . ARROW_FIRST . '</a>';
        }
        ...
        if (($paged + $half_pages_to_show) < ($max_page)) {
            echo '<a class="flecder" href="' . str_replace('&', '&amp;', get_pagenum_link($max_page)) . '">' . ARROW_LAST . '</a>';
        }
        echo "</span></div> $after";
    }
}
...
function get_property_price($postid) {
    if (get_post_meta($postid, 'price', true) && get_post_meta($postid, 'price', true) > 0) {
        ...
        return number_format(str_replace(".", "", get_post_meta($postid, 'price', true)), 0, ",", ".") . get_currency_sym();
    }
}
...
function get_property_price_info($pro_type='') {
    ...
    if ($option_value['list_type_title1']) {
        ...
        $info['is_feature'] = ($_SESSION['property_info']["property_list_type"] == "Featured" ? "Feature" : "");
...
function remove_from_favorite($property_id) {
    ...
    update_usermeta($userdata->ID, 'user_favorite_property', $user_meta_data);
    echo '<a href="javascript:void(0);" class="addtofav" onclick="javascript:addToFavorite(\'' . $property_id . '\',\'add\');"><img src="/wp-content/themes/RealEstate/images/ico-corazon.png">Add To Favorite</a>';
}
...
function favourite_html($user_id, $post_id) {
    ...
    if ($user_meta_data && in_array($post_id, $user_meta_data)) {
        ?>
        <span id="favorite_property_<?php echo $post_id; ?>" class="fav"  ><a href="javascript:void(0);" class="addtofav" onclick="javascript:addToFavorite(<?php echo $post_id; ?>,'remove');"><img src="<?php bloginfo('template_directory'); ?>/images/ico-corazon.png"/><?php _e('Favorited '); ?><?php _e(' <b>(remove)</b>'); ?></a>   </span>
<?php
    } else {
?>
...
function is_reserved_property($pid) {
    $temp = get_post_meta($pid, "reserved", true);
    if (!empty($temp) && $temp == true) {
        echo ' <img src="';
        bloginfo('template_directory');
        echo '/images/i-icon-reserved.png" alt="" title="" class="new" />';
    }
}

function is_reduced_property($pid) {
    $temp = get_post_meta($pid, "reduced", true);
    if (!empty($temp) && $temp == "true") {
        echo ' <img src="';
        bloginfo('template_directory');
        echo '/images/i-icon-reduced.png" alt="" title="" class="new" />';
    }
}
...
<?php is_reserved_property($post->ID); ?>
<?php is_reduced_property($post->ID); ?>
...
?>
            <?php $cat_info_arr = get_property_cat_id_name($post->ID); ?>
                ...
                <p class="propertylistinglinks">
                    <span>
                        <a href="<?php the_permalink(); ?>"><?php _e(VIEW_MORE_DETAILS_TEXT); ?>  <img src="<?= bloginfo("template_directory") ?>/images/ico-flechita-negro.gif"/></a>
                    </span>
<?php favourite_html($post->post_author, $post->ID); ?>
    ...
    <script language="javascript" type="text/javascript">
        ...
        $("#favorite_property_<?= $post->ID ?> a.jQueryBookmark").click(function(e){
