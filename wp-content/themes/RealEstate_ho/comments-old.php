...
<?php if ($comments) : ?>
	...
	<h3><?php comments_number(''.get_option('ptthemes_comment_responsesa_name').'', ''.get_option('ptthemes_comment_responsesb_name').'', ''.get_option('ptthemes_comment_responsesc_name').'' );?> &rarr; &#8220;<?php the_title(); ?>&#8221;</h3>
...
<div id="respond">
    ...
    <h3><?php echo get_option('ptthemes_comment_reply_name'); ?></h3>
    ...
    <?php else : ?>
        ...
        <form action="<?php echo get_option('siteurl'); ?>/wp-comments-post.php" method="post" id="commentform">
            ...
            <?php if ( $user_ID ) : ?>
                ...
                <p><?php echo get_option('ptthemes_comment_loggedin_name'); ?> &rarr; <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="Log out of this account"><?php echo get_option('ptthemes_comment_logout_name'); ?> &raquo;</a></p>

            <?php else : ?>

                <p class="commpadd"><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" />
				<label for="author"><small><?php echo get_option('ptthemes_comment_name_name'); ?> <?php if ($req) _e('*'); ?></small></label></p>

                <p class="commpadd"><input type="text" name="email" id="email" value="<?php echo $comment_auth_email; ?>" size="22" tabindex="2" />
				<label for="email"><small><?php echo get_option('ptthemes_comment_mail_name'); ?> <?php if ($req) _e('*'); ?></small></label></p>

				<p class="commpadd"><input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="22" tabindex="3" />
			    <label for="url"><small><?php echo get_option('ptthemes_comment_website_name'); ?></small></label></p>
