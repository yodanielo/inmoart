...
function custom_list_authors_count($args = '',$params = array()) {
    ...
    if($params['sort']=='most') {
        ...
        $sql = "SELECT count(ID) as user_count from $wpdb->users u join $wpdb->usermeta um on um.user_id=u.ID WHERE " . ($exclude_admin ? "user_login <> 'admin' " : '1')." and meta_key LIKE \"wp_capabilities\" AND meta_value LIKE '%s:5:\"agent\";%'" ;
...
function custom_list_authors($args = '',$params = array()) {
    ...
    if($params['sort']=='most') {
        ...
        $sql = "SELECT u.ID, u.user_nicename from $wpdb->users u join $wpdb->usermeta um on um.user_id=u.ID WHERE " . ($exclude_admin ? "user_login <> 'admin' " : '1')." and meta_key LIKE \"wp_capabilities\" AND meta_value LIKE '%s:5:\"agent\";%'" ;
        ...
        $sql .= " ORDER BY display_name limit $startlimit,$endlimit";
    }
...
function get_property_info_li($post) {
    ...
    <?php number_format(str_replace(".","",get_property_listing_price($post->ID)),0,",",".");?>
            </h3>
